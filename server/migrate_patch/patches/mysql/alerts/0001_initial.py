...
# 问题: MySQL 不支持在 JSON 列上创建索引 (错误码 3152: JSON column cannot be used in key specification)
#       PostgreSQL 专属索引 GinIndex/BTreeIndex 在 MySQL 上不支持
# 处理策略:
#   - BTreeIndex (datetime字段): 因 db_index=True 已创建索引，使用 FakeAddIndex 跳过避免重复
#   - GinIndex (JSONField字段):  使用 FakeAddIndex 跳过（MySQL JSON 列不支持直接索引）
# 受影响索引:
...
class FakeAddIndex(migrations.AddIndex):
    """跳过 JSONField 上的 GinIndex，MySQL 不支持 JSON 列直接索引"""
...
class Migration(migrations.Migration):
    ...
    operations = [
        ...
        # incident.operator: JSONField 在 MySQL 不支持直接索引 (错误码 3152)
        FakeAddIndex(
            model_name="incident",
        ...
        # --- 原 GinIndex(JSONField) → FakeAddIndex 跳过 ---
        # event.labels: JSONField 在 MySQL 不支持直接索引 (错误码 3152)
        FakeAddIndex(
            model_name="event",
        ...
        # --- 原 GinIndex(JSONField) → FakeAddIndex 跳过 ---
        # alert.operator: JSONField 在 MySQL 不支持直接索引 (错误码 3152)
        FakeAddIndex(
            model_name="alert",
